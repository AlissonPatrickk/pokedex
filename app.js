  ...
  Promise.all(pokemonPromises)
    .then(pokemons => {
      ...
      const listPokemons = pokemons.reduce((accumulator, pokemon) => {
        ...
        <li class="card ${types[0]}">
        <img class="card-image ${types[0]}" alt="${pokemon.name}" src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/shiny/${pokemon.id}.png" />
        <h2 class="card-title">${pokemon.id}. ${pokemon.name} Shiny</h2>
        <p class="card-subtitle">${types.join(' | ')}</p>
      </li>`
